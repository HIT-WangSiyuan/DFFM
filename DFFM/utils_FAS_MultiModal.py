...
def performances_FAS_MultiModal_C_W(CASIA_SURF_CeFA_val_filename, CASIA_SURF_CeFA_test_filename, WMCA_test_filename):
    # val 
    ...
    for line in lines:
        ...
        if label==1:
            ...
            num_fake += 1
    #AUC socre
    AUC_CASIA_SURF_CeFA=roc_auc_score(test_labels,test_scores)
    ...
    for line in lines:
        ...
        if label==1:
            ...
            num_fake += 1
    #AUC
    AUC_WMCA = roc_auc_score(test_labels, test_scores)
    # test based on val_threshold     
    type1 = len([s for s in data if s['map_score'] <= val_threshold and s['label'] == 1])
    type2 = len([s for s in data if s['map_score'] > val_threshold and s['label'] == 0])
    ...
    return ACER_CASIA_SURF_CeFA, TPR_FPR0001, ACER_WMCA, test_APCER, test_BPCER, TPR_FPR001,AUC_CASIA_SURF_CeFA,AUC_WMCA

def performances_FAS_MultiMoodal_intra(test_filename,val_filename):
    with open(val_filename, 'r') as file1:
        lines = file1.readlines()
    val_scores = []
    val_labels = []
    data = []
    ...
    test_APCER = type2 / num_fake
    test_BPCER = type1 / num_real
    ACER = (test_APCER + test_BPCER) / 2.0
    TPR_FPR0001 = tpr_fpr0001_funtion(test_labels, test_scores)
    return test_APCER,test_BPCER,ACER,AUC,TPR_FPR0001
...
def performances_FAS_MultiModal(CASIA_SURF_CeFA_val_filename, CASIA_SURF_CeFA_test_filename, WMCA_test_filename, WMCA_test_fakehead_filename, WMCA_test_flexiblemask_filename, WMCA_test_glasses_filename, WMCA_test_papermask_filename, WMCA_test_print_filename, WMCA_test_replay_filename, WMCA_test_rigidmask_filename):
    ...
    return ACER_CASIA_SURF_CeFA, TPR_FPR0001, ACER_WMCA, test_APCER, test_BPCER, TPR_FPR001, APCER_fakehead, APCER_flexiblemask, APCER_glasses, APCER_papermask, APCER_print, APCER_replay,  APCER_rigidmask ,AUC_CASIA_SURF_CeFA,AUC_WMCA
