...
landmarks_frame=pd.read_csv(os.path.join(root_dir,train_CASIA_SURF_CeFA_list), delimiter=' ', header=None)
for idx in range(len(landmarks_frame)):
    videoname = str(landmarks_frame.iloc[idx, 0])
    image_path = os.path.join(root_dir, videoname)
    if  not os.path.exists(image_path):
        print(image_path)
    videoname_depth = str(landmarks_frame.iloc[idx, 1])
    image_path_depth = os.path.join(root_dir, videoname_depth)     
    if  not os.path.exists(image_path_depth):
        print(image_path_depth)
    videoname_ir = str(landmarks_frame.iloc[idx, 2])
    image_path_ir = os.path.join(root_dir, videoname_ir)
    if  not os.path.exists(image_path_ir):
        print(image_path_ir)